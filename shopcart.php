                                    ...
                                    if ($_GET["action"] == "remove") //When customer clicks on the delete icon in the cart
                                    {
                                        foreach ($_SESSION["shoppingCart"] as $items => $values) {
                                            if ($values["product_item"] == $_GET["removeFromCart"]) {
                                                ...
                                                ?>
                                                <script>
                                                    window.location.href = "shopcart.php"; //Refresh
                                                </script>
                                                <?php
                                            }
                                        }
...
if (isset($_POST["cartSummary"])) //When the customer clicks the Add to Cart button
{
    if (isset($_SESSION["shoppingCart"])) //Keeps track of the shopping cart
    {
        $array_item_id = array_column($_SESSION["shoppingCart"], "product_item"); //creates sequential array for matching array keys to products id's
        ...
        if (!in_array($_GET["cartProd"], $array_item_id)) {
            ...
            $array_item = array(
                'product_item' => $_GET["cartProd"],
                'product_img' => $_POST["hiddenprodImg"],
                'brand_name' => $_POST["hiddenprodBrand"],
                'product_name' => $_POST["hiddenprodName"],
                'product_tax' => $_POST["prod_tax"],
                'product_price' => $_POST["hiddenprodPrice"],
            ...
            ?>
            <script>
                window.location.href = "shopcart.php"; //Refresh page
            </script>
            <?php
        } else { //product already exists, increase quantity
            ...
            for ($i = 0; $i < count($array_item_id); $i++) {
                if ($array_item_id[$i] == $_GET["cartProd"]) {
                    //add item quantity to the existing product in the array
                    $_SESSION['shoppingCart'][$i]['product_qty'] += $_POST["prodQty"];
                }
            }
        }
    } else //Nothing to add
...
<div class="bg0 p-tb-100">
    <div class="container">
        <form method="post" target="_self" name="cartForm" id="cartForm" action="checkout.php">
            <div class="wrap-table-shopping-cart">
                <table class="table-shopping-cart">
                    <tr class="table_head bg12">
                        <th class="column-1 p-l-30">Brand</th>
                    ...
                    foreach ($_SESSION['shoppingCart'] as $key => $product) {
                        ...
                        <tr class="table_row">
                            ...
                            <td style="text-align:center;" class="column-4">
                                <a href="shopcart.php?action=remove&removeFromCart=<?php echo $product['product_item']; ?>">
                                        <span class="text-danger">
                                            X
                                        </span>
                                </a>
